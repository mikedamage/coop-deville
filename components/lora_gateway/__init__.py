...
REMOTE_SENSOR_SCHEMA = sensor.sensor_schema().extend(
    {
        cv.Required(CONF_KEY): cv.string,
    }
)

REMOTE_BINARY_SENSOR_SCHEMA = binary_sensor.binary_sensor_schema().extend(
    {
        cv.Required(CONF_KEY): cv.string,
    }
)
...
REMOTE_NODE_SCHEMA = cv.Schema(
    {
        ...
        cv.Optional(CONF_SENSORS, default=[]): cv.ensure_list(REMOTE_SENSOR_SCHEMA),
        cv.Optional(CONF_BINARY_SENSORS, default=[]): cv.ensure_list(
            REMOTE_BINARY_SENSOR_SCHEMA
        ),
    }
)
...
async def to_code(config):
    ...
    for node_config in config[CONF_REMOTE_NODES]:
        ...
        for sens_conf in node_config[CONF_SENSORS]:
            sens = await sensor.new_sensor(sens_conf)
            cg.add(node_var.add_sensor(sens_conf[CONF_KEY], sens))

        for bsens_conf in node_config[CONF_BINARY_SENSORS]:
            bsens = await binary_sensor.new_binary_sensor(bsens_conf)
            cg.add(node_var.add_binary_sensor(bsens_conf[CONF_KEY], bsens))
